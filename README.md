...
여러가지의 샘플을 돌려 본 결과 모델별로 약간의 차이가 있다는 것을 알 수 있었다. 세 모델 모두 가로, 세로 픽셀을 4배씩 늘려 그 안의 색을 조절한다는 방식은 같지만 EDSR과 WDSR은 주변과  뭉그뜨려 변환 되는 반면 SRGAN을 사용했을 때 더 세세한 부분까지 표현이 된다는 것을 알 수 있었다. 때문에 픽셀 수 자체는 16배 늘어 크기가 동일하지만, 우리가 느끼기에 SRGAN의 결과물이 가장 좋다고 생각한다. EDSR과 WDSR이 차이가 많이 안나는 이유는 WDSR이 다중 이미지 복원 특화되어있는 모델인데 우리가 잡은 주제 자체가 단일 이미지에 대한 복원에 관련된 내용이므로 WDSR의 특징을 뚜렷하게 느낄 수 없는게 아닌가 싶다.

우리가 찍은 사진들 중 글씨가 존재하는 사진은 해상도를 높여도 흐린 글씨가 다시 읽을 수 있게 되지는 않았다. 그림에서의 super resolution은 우리가 아주 세세한 부분까지 구분을 할 필요가 없기 때문에 전체적인 해상도가 올라간다는 느낌을 주지만 만약 글자 이미지라면 해상도를 높여도 우리가 구분해야하는 글자 자체가 이미 작고 픽셀을 많이 차지하는게 아니기 때문에 애초에 구분하기 쉽지 않은 것 같다.

가장 큰 문제점은 큰 사이즈, 즉 픽셀이 매우 많은 이미지는 실행을 하기 어렵다는 것이다. 픽셀 수 자체가 16배 씩 많아지기 때문에 이미지의 사이즈가 커지면 커질 수로 계산에 걸리는 시간이 상당히 많이 걸리는 것 같았다. 테스트 결과 CPU가 i5-8500인 컴퓨터에서 최대 500X500 사이즈인 이미지의 해상도를 올릴 수 있다는 것으로 나왔다. 더 좋은 환경에서 테스트 해보진 못했지만 더욱 극적인 차이를 만들기 위해서는 매우 좋은 CPU에서 작동을 해야할 것 같다.
